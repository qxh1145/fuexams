...
export const signIn = async (req, res) => {
    try {
        ...
        if(!username || !password){
            return res.status(400).json({message: "Please in put username and password"});
        }
        ...
        return res.status(200).json({message: `User ${user.displayName} have logged in`, accessToken})
    } catch (error) {
